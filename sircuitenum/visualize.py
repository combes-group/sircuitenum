...
def draw_port_graph(circuit: list, edges: list, out="port_graph.png",
    ...
    Args:
        ...
        scale (float, optional): size for tuning overall spacing of nodes in plot
...
def draw_basegraph(G: nx.Graph, title:str = "", savename: str = "", pos:dict={}):
    """Plots unlabeled graphs from graph list returned
    from get_graphs_from_file() function
...
def draw_all_circuits(file: str, n_nodes: int, out_dir: str,
    ...
    for uid, row in tqdm(df.iterrows(),total=df.shape[0]):
        ...
        viz.draw_circuit_diagram(circuit, edges, out=Path(out_dir,f"{uid}{format}"),
                         layout=layout,graph_index=graph_index)
        plt.close()


def draw_circuit_diagram(circuit: list, edges: list, out: str = "circuit_diagram.png",
             scale: float = 4.0, layout:str='planar', spread:float=2/5, graph_index:int=None):
    """
    Draws the circuit diagram using schemdraw.

    For parallel elements goes to 1/4 of the way along the connection and fans out
    to do the elements in parallel.
    ...
    Args:
        ...
        layout (str): options for graph layouts. 
                      Spring seems to produce prettier circuits, but it often results in 
    ...
    if layout == 'planar':
        ...
        pos = nx.spring_layout(G)
    elif layout == 'fixed' and not(graph_index is None):
        pos = G_POS[str(G.number_of_nodes())][graph_index]
    else:
        raise ValueError("must specify layout for graph and include graph_index if using fixed layout")
                        ...
                        if nEdges % 2 == 0:
                            ...
                            for edgeNum in range(int(nEdges/2)):
                                ...
                                d.add(elem_bank[edge1['element']]().endpoints(
                                    a0+perp_step*(edgeNum+1/2), a1+perp_step*(edgeNum+1/2)))
                            ...
                            for edgeNum in range(1, int((nEdges-1)/2 + 1)):
                                ...
                                d.add(elem_bank[edge1['element']]().endpoints(
                                    a0+perp_step*edgeNum, a1+perp_step*edgeNum))
                                d.add(elem_bank[edge2['element']]().endpoints(
                                    a0-perp_step*edgeNum, a1-perp_step*edgeNum))
        if out != "":
            d.save(out)
...
if __name__ == "__main__":
    ...
    # Basegraphs
    base = f'/home/eweissler/img/basegraphs'
    pos = {}
    for n_nodes in range(2,5):
        pos[str(n_nodes)] = []
        all_graphs = utils.get_basegraphs(n_nodes)
        for i, G in enumerate(all_graphs):
            f, p = draw_basegraph(G,f"{n_nodes} nodes, graph {i}",str(Path(base,f"n{n_nodes}_g{i}.svg")),
                    G_POS[str(n_nodes)][i])
            for k in p:
                p[k] = np.round(p[k],2)
    ...
    # Actual Circuits
    # base = f'/home/eweissler/img/planar_layout'
    # toLoad = '/home/eweissler/src/test/circuits_trimmed.db'
    # for n_nodes in range(2,5):
    #     out_dir = Path(base,f'{n_nodes}_node_circuits')
    #     out_dir.mkdir(parents=True,exist_ok=True)
    #     viz.draw_all_circuits(toLoad, n_nodes, out_dir=out_dir, layout='planar')

    base = f'/home/eweissler/img/fixed_layout'
    toLoad = '/home/eweissler/src/test/circuits_trimmed.db'
    for n_nodes in range(2,5):
        out_dir = Path(base,f'{n_nodes}_node_circuits')
        out_dir.mkdir(parents=True,exist_ok=True)
        viz.draw_all_circuits(toLoad, n_nodes, out_dir=out_dir, layout='fixed')