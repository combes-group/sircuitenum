...
def test_single_edge_loop_knitting():
    ...
    c2, e2 = pi.single_edge_loop_kiting(circuit, edges)
    assert(c2 == [("C",),("C","J"),("C","L"), ("C","J"), ("L",),("L",),("C","L"), ("C",)])
    assert(e2 == [(0,1), (1,2), (2,3), (2,4), (2,5), (5,4), (3,4),(4,0)])
...
def test_gen_param_dict():
    ...
    for i in range(len(TEST_CIRCUITS)):
        ...
        assert(all(x == utils.ELEM_DICT['C']['default_value'] for x in param_dict['c_list']))
        assert(all(x == utils.ELEM_DICT['L']['default_value'] for x in param_dict['l_list']))
        assert(all(x == utils.ELEM_DICT['J']['default_value'] for x in param_dict['j_list']))
        assert(all(x == utils.ELEM_DICT['CJ']['default_value'] for x in param_dict['cj_list']))
